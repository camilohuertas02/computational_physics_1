...
double vprima(double x, double v, double omg, double t, 
    ...
    return gamma*cos(omg*t) - delta*v - alfa*x - beta*pow(x,3);
}
...
int main() {
	...
	v0 = new_v;
        x0 = new_x;
       	...
       	double new_a = vprima(x0, v0, omg, t0, gamma, delta, alfa, beta); 
	a0 = new_a;

        datafile << t0 << " " << x0 << " " << v0 << " " << a0 <<std::endl;
    ...
    scriptFile << "set term png\n"
	       ...
	       << "set ylabel 'x'\n"
               ...
               << "set xlabel 't'\n"
               << "set ylabel 'a'\n"
               << "plot 'duffing.txt' using 1:4 with linespoints\n"
               << "set output 'VvsT.png'\n"
               << "set title 'VvsT'\n"
               << "set xlabel 't'\n"
               << "set ylabel 'v'\n"
               << "plot 'duffing.txt' using 1:3 with linespoints\n";
    scriptFile.close();
